...
function mobile() {
  ...
  navMobile.setAttribute('aria-label', navMobile.classList.contains('open') ? 'ocultar menú' : 'mostrar menú');
  if(navShow.classList.contains('list-show')) {
    navShow.querySelector('li').setAttribute('tabIndex', '-1');
    navShow.querySelector('li').focus();
  } else {
    navMobile.focus();
  }
}
...
function getMlResult(e) {
  ...
  if ((mgProductInput === "" || mgProductInput === "0") || (mlProductInput === "" || mlProductInput === "0") || (mgDoseInput === "" || mgDoseInput === "0") || (catWeightInput === "" || catWeightInput === "0")) {
    ...
    // qué campos reviso si no los veo
    ...
    modalText.innerHTML = ((weightDose * mlProductInput) / mgProductInput)
      .toFixed(3)
      .replace(".", ",") + " ml";
